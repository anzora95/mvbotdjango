...
def NewFollowLike(request):
    ...
    if request.method == 'POST':
        ...
        if request.POST.get('friend')!="friend":
            ...
            if request.POST.get('foll')=='True':
                task.follwers_user=True
            else:
                task.followeds_user=True
...
def start(request, task):
    ...
    ft_back=task.back
    ft_followings=task.followeds_user
    ft_followers=task.follwers_user
    ...
    print(hl)
    user=User.objects.get(id=request.user.id)
    runbot.delay(u,p,hl,ide,ftLike,ftFollow,ftUnfollow,pa,n_ceil,friend,ft_all,ft_back, ft_followers,ft_followings) #aqui debe de haber otro parametro que sea lo de el friend list para que se inicialize en instabot
    return redirect('instabot:dashboard')
